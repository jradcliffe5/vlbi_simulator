...
def calc_sefd(sefd1,sefd2,tint,dnu,sampeff):
	...
	return (1./(float(sampeff)))*(sefd_a)*(1./(np.sqrt(2*tint*dnu)))

def add_noise(msfile,datacolumn,evn_SEFD,adjust_time=1.0,complex_noise=False):
	tb = casatools.table()
	...
	for i in range(len(antenna1)):
		if complex_noise == True:
			...
			amps = np.random.normal(0.,sefd,np.shape(data[:,:,i]))+1j * np.random.normal(0.,sefd,np.shape(data[:,:,i]))
			data[:,:,i]=amps
