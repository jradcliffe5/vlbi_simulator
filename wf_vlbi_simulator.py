...
if part == 1:
	...
	commands.append('%s -name %s/%s_single_pointing -no-update-model-required --aterm-kernel-size 157 -weight %s -scale %s -niter 1 -mgain 0.9 -auto-threshold 0.5 -auto-mask 4 -use-idg -idg-mode hybrid -aterm-config single_pointing.ms_aterm_norotate_config.txt -size %d %d %s/%s_single_pointing.ms'%(inputs['wsclean_exec'],inputs['output_path'],inputs['prefix'],inputs['weight'],inputs['cell'],inputs['prefix'],int(inputs['size']),int(inputs['size']),inputs['output_path'],inputs['prefix']))

	if inputs['mosaic'] == "False":
		commands.append('%s %s/single_pointing-image-pb.fits'%(inputs['rms_exec'],inputs['output_path']))
			
	else:
		commands.append('%s %s/simulations/fit_pb.py'%(rpath, inputs['CASA_exec']))
		commands.append('%s %s/simulations/generate_mosaic_pointings.py'%(rpath, inputs['CASA_exec']))
